...
class YOLOFaceDetector:
    def __init__(self):
        ...
        self.processed_image_pub = rospy.Publisher('/face_detection/image/compressed', CompressedImage, queue_size=1)
        self.target_pub = rospy.Publisher('/face_detection/targets', Target, queue_size=1)
    ...
    def _process_frames(self):
        ...
        while not rospy.is_shutdown():
            try:
                ...
                compressed_msg.data = np.array(cv2.imencode('.jpg', cv_image, [cv2.IMWRITE_JPEG_QUALITY, 60])[1]).tobytes()
                ...
                if boxes is not None:
                    ...
                    print(f"Обнаружено {len(boxes)} лиц")
                else:
